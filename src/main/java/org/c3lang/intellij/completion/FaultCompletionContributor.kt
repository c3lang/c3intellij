...
object FaultCompletionContributor : CompletionProvider<CompletionParameters>() {
    ...
    override fun addCompletions(
        ...
        val insertHandler = FaultInsertHandler(
        ...
        }.filterIsInstance<C3FaultDefinition>().forEach { faultDefinition ->
            val fullyQualifiedName = FullyQualifiedName.from(faultDefinition)

            val lookupElementBuilder = LookupElementBuilder
                .create(faultDefinition, fullyQualifiedName.fullName)
                .withLookupStrings(
                    listOf(
